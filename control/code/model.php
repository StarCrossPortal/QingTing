...
function uploadData($serverAddr, $usceId, $token, $nodeId)
{
    addlog("开始将节点数据上传到服务器");

    $header = ['Content-Type' => 'application/json;charset=UTF-8'];
    $url = "{$serverAddr}/user_api/upload_data_batch.html?usce_id={$usceId}&node_id={$nodeId}";
    while (true) {
    ...
    while (true) {
        ...
        foreach ($list as $val) {
            ...
            if (empty($info)) {
                ...
                $result = Db::table('target')->replace()->strict(false)->insertGetId($val);

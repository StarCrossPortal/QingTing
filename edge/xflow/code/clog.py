...
#     if len(container_id[1].split("_")[2])<8:
#         xflow_node_id='_'.join(container_id[1].split("_")[0:2])
#     else:
#         xflow_node_id=container_id[1].split("_")[2]
    xflow_node_id = container_id[1]
    # 从对应的容器获取日志内容
    try:
            ...
            while True:
                ...
                if len(read) > 1:
                    ...
                    try:
                        # 开启事务
                        mutex.acquire()
                        if container_names !='mysql':
                            cursor.execute(
                                "replace into contain_log (flow_id,contain_names,contain_id,info,hash,xflow_node_id) values(%s,%s,%s,%s,%s,%s)",
                                (xflow_id, container_names, container_id[0], read,hash,xflow_node_id))
                        mutex.release()

...
function main()
{
    ...
    foreach ($list as $item) {
        ...
        if (empty($data['raw_data'])) {
            addLog(["xscan扫描目标后返回结果为空", $data['raw_data']]);
            continue;
        }

        $data['xflow_node_id'] = $xflowNodeId;
        $data['hash'] = md5(json_encode($data));
        $bbb = Db::table('xscan')->strict(false)->extra("IGNORE")->insert($data);
        var_dump($bbb);
    }

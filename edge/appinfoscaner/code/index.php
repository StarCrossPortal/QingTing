...
// 循环读取状态值,直到执行完成
    while (true) {
        ...
        $result = Db::table('control')->where(['ability_name' => 'appinfoscaner', 'status' => 1])->find();
        if (empty($result)) {
            sleep(15);
            continue;
        }
        addlog("appinfoscaner 开始工作");

        //读取目标数据,排除已经扫描过的目标
        $targetArr = Db::table('target')->where('id', 'NOT IN', function ($query) {
            $query->table('scan_log')->where(['tool_name' => 'appinfoscaner', 'target_name' => 'target'])->field('data_id');
        ...
        foreach ($targetArr as $value) {
            ...
            //
            $pathArr = getSavePath($url, "appinfoscaner", $id);
            $inputType = "url";
        ...
        //更新最后扫描的ID
        updateScanLog('appinfoscaner', 'target', $value['id'] ?? 0);

        // 修改插件的执行状态
        Db::table('control')->where(['ability_name' => 'appinfoscaner'])->update(['status' => 0, 'end_time' => date('Y-m-d H:i:s')]);

        addlog("appinfoscaner执行完毕");
        sleep(20);
    }
...
{
    $result = [];
    addlog(["appinfoscaner开始执行扫描任务", $id, $url]);
    $path = "cd /data/tools/appinfoscaner/ && ";
    ...
    $result = implode("\n", $result);
    addlog(["appinfoscaner漏洞扫描结束", $tid, $url, $cmd, base64_encode($result)]);
    $result = file_put_contents($pathArr['cmd_result'], $result);
    if ($result == false) {
        addlog(["appinfoscaner写入执行结果失败", base64_encode($pathArr['cmd_result'])]);
    }
}
...
function writeData($pathArr, $url, $value)
{
    ...
    if (!file_exists($pathArr['tool_result'])) {
        addlog("appinfoscaner 扫描结果文件不存在:{$pathArr['tool_result']},URL {$url}");
    }
    //读取命令写入文件的执行结果
    if (!file_exists($pathArr['cmd_result'])) {
        addlog("appinfoscaner 扫描结果文件不存在:{$pathArr['cmd_result']},URL {$url}");
    }
    ...
    Db::table('appinfoscaner')->extra('IGNORE')->insert($newData);

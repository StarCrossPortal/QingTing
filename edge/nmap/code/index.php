...
// 循环读取状态值,直到执行完成
    while (true) {
        ...
        $targetArr = Db::table('target')->where('id', 'NOT IN', function ($query) {
            $query->table('scan_log')->where(['tool_name' => 'nmap', 'target_name' => 'target'])->field('data_id');
        ...
        //更新最后扫描的ID
        updateScanLog('nmap', 'target', $value['id'] ?? 0);
...
function execTool($id, $url, $tid, $pathArr)
{
    ...
    // 通过系统命令执行工具
    $cmd = "{$path} nmap -sS -Pn -T4  $url | grep open | grep -v Discovered |grep -v grep  2>&1";
    execLog($cmd, $result);

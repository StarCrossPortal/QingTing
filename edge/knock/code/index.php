...
// 循环读取状态值,直到执行完成
    while (true) {
        ...
        $result = Db::table('control')->where(['ability_name' => 'knock', 'status' => 1])->find();
        if (empty($result)) {
            sleep(15);
            continue;
        }
        ...
        $targetArr = Db::table('target')->where('id', 'NOT IN', function ($query) {
            $query->table('scan_log')->where(['tool_name' => 'knock', 'target_name' => 'target'])->field('data_id');
        ...
        foreach ($targetArr as $value) {
            ...
            //
            $pathArr = getSavePath($url, "knock", $id);
            $inputType = "domain";
        ...
        //更新最后扫描的ID
        updateScanLog('knock', 'target', $value['id'] ?? 0);

        // 修改插件的执行状态
        Db::table('control')->where(['ability_name' => 'knock'])->update(['status' => 0, 'end_time' => date('Y-m-d H:i:s')]);
...
function execTool($id, $url, $tid, $pathArr)
{
    ...
    $result = implode("\n", $result);
    addlog(["knock漏洞扫描结束", $tid, $url, $cmd, base64_encode($result)]);
    $result = file_put_contents($pathArr['cmd_result'], $result);
    if ($result == false) {
        addlog(["knock写入执行结果失败", base64_encode($pathArr['cmd_result'])]);
    }
}
...
function writeData($pathArr, $url, $value)
{
    ...
    if (!file_exists($pathArr['tool_result'])) {
        addlog("knock 扫描结果文件不存在:{$pathArr['tool_result']},URL {$url}");
    }
    //读取命令写入文件的执行结果
    if (!file_exists($pathArr['cmd_result'])) {
        addlog("knock 扫描结果文件不存在:{$pathArr['cmd_result']},URL {$url}");
    }
    ...
    Db::table('knock')->extra('IGNORE')->insert($newData);

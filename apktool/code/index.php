...
function main()
{
    ...
    foreach ($list as $item) {
        ...
        $data = execTool($itemData[$urlField]);

        if (empty($data['raw_data'])) {
            addLog(["apktool扫描目标后返回结果为空", $data['raw_data']]);
            continue;
        }

        $data['xflow_node_id'] = $xflowNodeId;
        $data['hash'] = md5(json_encode($data));
        $bbb = Db::table('apktool')->strict(false)->extra("IGNORE")->insert($data);
        var_dump($bbb);
    }
    ...
    // 修改插件的执行状态
    Db::table('control')->where(['ability_name' => 'apktool','task_version'=>$params['task_version']])->update(['status' => 0, 'end_time' => date('Y-m-d H:i:s')]);

    addlog("apktool 脚本 执行完毕");
}
...
function execTool($url)
{
    ...
    $path = "cd /home/appuser/tool/ && ";

    // 通过系统命令执行工具
    $cmd = "{$path} ./apktool d {$url} -o output_file";
    exec($cmd, $result);
